...
-- Idempotent: UPDATE is guarded inside the constraint-existence check so
-- re-runs against a DB that already has the constraint are a no-op.
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'orders_status_check') THEN
    -- Migrate legacy status values to canonical names before adding the CHECK constraint.
