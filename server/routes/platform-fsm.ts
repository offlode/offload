...
export function registerFsmRoutes(app: Express) {
  ...
  // ── Payment helpers needed by FSM cancel + partial-refund ──
  ...
  // Export issueStripeRefundForOrder so platform-comms.ts can use it for partial-refund
  // We achieve this by returning it from registerFsmRoutes — callers that need it import this module.
  // However, since we need to keep the architecture simple (no circular deps), we duplicate the
  // payment helper in platform-comms.ts for the partial-refund endpoint.
  // The FSM cancel path uses the local copy above.
